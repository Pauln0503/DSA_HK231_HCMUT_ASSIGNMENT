...
public:
    ...
    V remove(K key, void (*deleteKeyInMap)(K)=0)override;
    bool remove(K key, V value, void (*deleteKeyInMap)(K)=0, void (*deleteValueInMap)(V)=0)override;
    bool containsKey(K key)override;
    bool containsValue(V value)override;
    bool empty()override;
    int size()override;
    void clear()override;
    string toString(string (*key2str)(K&)=0, string (*value2str)(V&)=0 )override;
    DLinkedList<K> keys()override;
    DLinkedList<V> values()override;
    DLinkedList<int> clashes()override;
...
//     this -> shouldRemoveInternalData = true;
//  }

template <class K, class V>
 xMap<K, V>::xMap(int (*hashCode)(K &, int), float loadFactor, bool (*valueEqual)(V &, V &), void (*deleteValues)(xMap<K, V> *), bool (*keyEqual)(K &, K &), void (*deleteKeys)(xMap<K, V> *))
{
    this->loadFactor = loadFactor;
    this->hashCode = hashCode;
    this->valueEqual = valueEqual;
...
{
    //YOUR CODE IS HERE
    this -> shouldRemoveInternalData = false;
    //this ->copyMapFrom(map);
    if (shouldRemoveInternalData) {
        removeInternalData();
    }
    ...
    // Không sao chép deleteKeys, deleteValues để chỉ xóa một lần khi cần
    
    // Sao chép các entry từ map nguồn
...
V xMap<K,V>::put(K key, V value) {
    ...
    V retValue = value;
    DLinkedList<Entry*>& entry_list = this->table[index]; // Lấy danh sách từ bảng băm tại chỉ số index

    // Đây là chỗ tìm key
    for (auto entry : entry_list) {
        if (keyEQ(entry->key, key)) { // Nếu key đã tồn tại
            retValue = entry->value; // Lưu giá trị cũ vào retValue
            entry->value = value; // Cập nhật giá trị mới
            return retValue; // Trả về giá trị cũ
        }
    }
    // Nếu key chưa tồn tại, tạo Entry mới và thêm vào danh sách
    Entry* new_entry = new Entry(key, value);
    entry_list.add(new_entry); // Thêm cặp <key, value> vào danh sách
    count++; 
...
V& xMap<K,V>::get(K key){
    ...
    //V retValue = value;
    //YOUR CODE IS HERE   
    auto& entryList = table[index]; // Lấy danh sách tại chỉ số index

    // đây là chỗ tìm key
    for (auto entry : entryList) {
        if (keyEQ(entry->key, key)) { // Nếu tìm thấy key
            return  entry->value; 
        }
    }
    //key: not found
...
V xMap<K,V>::remove(K key, void (*deleteKeyInMap)(K)) {
    int index = hashCode(key, capacity);
    DLinkedList<Entry*>& entry_list = this->table[index]; 
    int pos = 0; // Khởi tạo biến để theo dõi vị trí
  V retValue;
    // Tìm key có trong danh sách
    for (auto it = entry_list.begin(); it != entry_list.end(); ++it, ++pos) {
        if (keyEQ((*it)->key, key)) { // Nếu tìm thấy key
             retValue = (*it)->value; // backup value để trả về

            // Giải phóng key nếu deleteKeyInMap khác nullptr
            if (deleteKeyInMap) {
                deleteKeyInMap((*it)->key);
            }

            // Gỡ Entry (chứa cặp <key, value>) ra khỏi danh sách
            entry_list.removeAt(pos); // Sử dụng vị trí để gỡ bỏ mục
            //deleteEntry(*it); // Giải phóng vùng nhớ của Entry (nếu cần)

            count--; // Giảm kích thước khi gỡ bỏ
            return retValue; // trả về giá trị backup
        }
    }
...
DLinkedList<K> xMap<K,V>::keys(){
    //YOUR CODE IS HERE 
    DLinkedList<K> key_list ;

    for (int i = 0; i < capacity; i++){
        DLinkedList<Entry*>& entry_list = table[i];

        for (auto entry : entry_list) {
            key_list.add(entry->key);  
        }
    }
    return key_list;
}
...
DLinkedList<V> xMap<K,V>::values(){
    //YOUR CODE IS HERE 
    DLinkedList<V> value_list;  
    for (int i = 0; i < capacity; i++) {
        DLinkedList<Entry*>& entry_list = table[i];  

        for (auto entry : entry_list) {
            value_list.add(entry->value);  
        }
    }
    return value_list; 
}
...
DLinkedList<int> xMap<K,V>::clashes(){
    ...
    for (int i = 0; i < capacity; i++) {
        DLinkedList<Entry*>& entry_list = table[i];  
        clash_list.add(entry_list.size()); 
    }
