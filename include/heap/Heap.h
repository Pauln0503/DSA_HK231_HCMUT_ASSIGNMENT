...
public:
    ...
    void remove(T item, void (*removeItemData)(T)=0)override;
    bool contains(T item)override;
    int size() override;
    void heapify(T array[], int size)override;
    void clear()override;
    bool empty()override;
...
void Heap<T>::remove(T item, void (*removeItemData)(T)){
    //YOUR CODE IS HERE
    int found_index = getItem(item);
    if (found_index == -1 || elements[found_index] != item) {
        return; 
    }
    elements[found_index] = elements[count - 1];
    count--;
    reheapDown(found_index);
...
void Heap<T>::reheapUp(int position){
    // Vòng lặp di chuyển phần tử lên cho đến khi nó nằm đúng vị trí
    while (position > 0) {
        int parent = (position - 1) / 2; // Tính chỉ số của phần tử cha
        // Nếu phần tử hiện tại nhỏ hơn (hoặc lớn hơn, nếu là max-heap) phần tử cha
        if (aLTb(elements[position], elements[parent])) {
            swap(position, parent); // Hoán đổi vị trí
            position = parent; // Cập nhật vị trí hiện tại
        } else {
            break; // Nếu không cần hoán đổi, thoát khỏi vòng lặp
        }
    }
}
...
void Heap<T>::reheapDown(int position){
    //YOUR CODE IS HERE
    int child;
    while ((child = position * 2 + 1) < count) { // Tìm con trái
        // Tìm con lớn hơn giữa con trái và con phải
        if (child + 1 < count && aLTb(elements[child + 1], elements[child])) {
            child++; // Chuyển sang con phải
        }
        // Nếu phần tử hiện tại lớn hơn phần tử con
