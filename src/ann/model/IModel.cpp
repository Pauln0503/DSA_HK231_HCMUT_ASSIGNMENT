...
void IModel::fit(DataLoader<double, double>* pTrainLoader,
    ...
    for(int epoch=1; epoch <= nepoch; epoch++){
        ...
        for (auto batch : *pTrainLoader) {
            ...
            //(0) Đưa tất cả các delta trong m_pOptimizer về 0
            m_pOptimizer->zero_grad();

            //(1) Tính Y bằng forward
            double_tensor Y = this->forward(X);

            //(2) Tính batch_loss là giá trị vô hướng bằng gọi hàm m_pLossLayer->forward
            double batch_loss = m_pLossLayer->forward(Y, t);

            //(3) Gọi backward để tính delta cho w và b (không cần truyền tham số)
            this->backward();

            //(4) m_pOptimizer->step cập nhật w và b theo delta của w và b
            m_pOptimizer->step();

            // Ghi nhận hiệu suất của từng batch
