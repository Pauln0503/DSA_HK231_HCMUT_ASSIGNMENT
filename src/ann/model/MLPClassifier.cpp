...
double_tensor MLPClassifier::predict(DataLoader<double, double>* pLoader, bool make_decision) {
    ...
    for(auto batch: *pLoader) {
        ...
        // Cập nhật results
        if(first_batch) {
            // Dự đoán kết quả cho batch đầu tiên
            auto shape = batch_output.shape();
            shape[0] = pLoader->get_sample_count(); // Cập nhật kích thước
            results = xt::zeros<double>(shape);     // Khởi tạo mảng kết quả
            first_batch = false;
        }

        // Cập nhật kết quả cho các batch tiếp theo
        xt::view(results, xt::range(nsamples, nsamples + X.shape()[0]), xt::all()) = batch_output;
